...
router.post('/',
  ...
  body('alt_mobile_no').optional({ checkFalsy: true }).trim(),
  body('address').optional({ checkFalsy: true }).trim(),
  body('salutation').optional({ checkFalsy: true }).trim(),
  body('color').optional({ checkFalsy: true }).trim(),
  body('pattern_password').optional({ checkFalsy: true }).trim(),
  body('imei_status').optional({ checkFalsy: true }).trim(),
  body('power_status').optional({ checkFalsy: true }).trim(),
  body('touch_status').optional({ checkFalsy: true }).trim(),
  body('display_status').optional({ checkFalsy: true }).trim(),
  body('device_condition').optional({ checkFalsy: true }).trim(),
  body('data_backup').optional({ checkFalsy: true }).trim(),
  body('estimated_amount').optional({ checkFalsy: true }).isFloat({ min: 0 }),
  body('advance_amount').optional({ checkFalsy: true }).isFloat({ min: 0 }),
  body('confirm_estimated').optional({ checkFalsy: true }).isBoolean(),
  body('prepared_by').optional({ checkFalsy: true }).trim(),
  body('technician').optional({ checkFalsy: true }).trim(),
  body('eta').optional({ checkFalsy: true }).isISO8601(),
  body('remarks').optional({ checkFalsy: true }).trim(),
  body('status').optional({ checkFalsy: true }).isIn(['Pending', 'In Progress', 'Ready for Delivery', 'Delivered', 'Returned', 'Delayed', 'Cancelled']),
  async (req, res) => {
